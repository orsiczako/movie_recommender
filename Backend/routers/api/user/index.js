    ...
    asyncHandler(async (req, res) => {
      ...
      if (!result.success) {
        const msg = result.error === 'INVALID_CURRENT_PASSWORD' 
          ? 'user.errors.invalid_current_password' 
          : 'user.errors.password_change_failed';
        return ApiResponse.error(res, msg, 400);
      }

      return ApiResponse.success(res, 'user.success.password_changed');
    })
  );
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!result.success) {
        const msg = result.error === 'USER_NOT_FOUND' 
          ...
          : 'user.errors.delete_failed';
        return ApiResponse.error(res, msg, result.error === 'USER_NOT_FOUND' ? 404 : 400);
      }

      return ApiResponse.success(res, 'user.success.account_deleted');
    })
  );
