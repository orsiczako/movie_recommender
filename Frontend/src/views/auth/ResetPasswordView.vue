    ...
    <div class="form-container">
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <button type="submit" :disabled="loading">
          {{ loading ? 'Loading...' : $t('auth.reset_password_button') }}
        </button>
...
export default {
  ...
  mounted() {
    ...
    if (!this.token) {
      this.errorMessage = this.$t('auth.invalid_token')
    }
  },
  methods: {
    async handleSubmit() {
      ...
      if (this.form.password !== this.form.confirmPassword) {
        this.errorMessage = this.$t('auth.passwords_dont_match')
        this.loading = false
        return
      }
      ...
      try {
        const result = await authService.resetPassword(this.token, this.form.password)
        
        if (result.success) {
          this.successMessage = this.$t('auth.password_reset_success')
          this.form.password = ''
          this.form.confirmPassword = ''
          
          // Redirect to login after 3 seconds
          setTimeout(() => {
            this.$router.push('/login?passwordReset=true')
          }, 3000)
        } else {
