  ...
  setup() {
    ...
    const fetchMovies = async () => {
      try {
        ...
        const userId = user.value?.id; // Use id consistently
    ...
    const performLike = async () => {
      ...
      try {
        const userId = user.value?.id; // Use id like FavoriteMoviesView
...
.movie-browsing-view {
  --action-red: var(--danger);
  --action-green: var(--success);
  --action-blue: var(--primary);
}

/* Override any hard-coded colors for better theme support */
.detail-section {
...
.theme-dark .quality-text,
.theme-dark .fact-text,
.theme-dark .movie-title,
.theme-dark .movie-overview {
  color: var(--text-primary) !important;
}

.theme-light .quality-text,
.theme-light .fact-text,
.theme-light .movie-title,
.theme-light .movie-overview {
  color: var(--text-primary) !important;
}
</style>
